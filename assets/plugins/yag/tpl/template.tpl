   									...
   									container:"yag_container",
									    ...
									    { type:"header", template:"[+tabName+]" },
												{view:"toolbar", elements:[
														...
														{view:"toggle", type:"iconButton",tooltip:"Отображение удаленных документов", offLabel:showButton, onLabel:hideButton, icon:"trash-o", width:110, click: function(){
															if(!showDeleted){
														...
														{ view:"select", id:"ops", label:"Действие:", labelAlign:"right", value:1, options:[{id:1, value:"Публиковать"},
																		   {id:2, value:"Не публиковать"},
																		   {id:3, value:"Удалить"}], width:200},
														//Кнопка выполнить выбранное действие
														{view:"button", type:"iconButton", label:"Применить",tooltip:"Применить выбранное действие", icon:"cog", width:110, click: function(){
															var chIds = [];
															$$$("grid").data.each(function(obj){
																															if(obj.ch){
																																deletedItems.push(obj);
																	...
																	switch($$$("ops").getValue()){
																	case "1":
																		webix.ajax().post("[+url+]?mode=publish",
																											 {ids:chIds},
																											 function(text){
																		...
																		break;
																	case "2":
																	 	webix.ajax().post("[+url+]?mode=unPublish",
																											 {ids:chIds},
																											 function(text){
    																											webix.message(text,"info");
																											});
																	 	break;
																	case "3":
																		webix.ajax().post("[+url+]?mode=delete",
																											 {ids:chIds},
																											 function(text){
    																											webix.message(text,"info");
														...
														//Кнопка Импорт
														{ view:"uploader", type:"iconButton", label:"Импорт", icon:"download", autowidth:true, on:{
														...
														//Кнопка Экспорт
														{view:"button", type:"iconButton", icon:"upload", label:"Экспорт", autowidth:true, click: function(){
															webix.toExcel($$$("grid"),
															{
																filename: "[+tabName+]",
																filterHTML:true,
														...
														{ view:"button", type:"iconButton", icon:"undo", label:"Undo", autowidth:true, click: function(){
															id = historyUndo.pop();
															if(!id) return;
															$$$('grid').undo(id);
															row = $$$('grid').getItem(id);
														...
														{view:"button", type:"iconButton", label:"Добавить",tooltip:"Добавить новый документ", icon:"plus-square-o", width:110, click: function(){
																$$$("newDoc").show();
														}}
														]},
											{view:"form",
									    ...
									    {cols:[
										    {
									    		...
									    		{view:"form",
									    		 padding:7,
									    		 cols:[{id:"pagerB", view:"pager",template:"{common.prev()} {common.pages()} {common.next()}"}]
									    	}
									  	]
				     						});
				//Pager под таблицей
